...
class ClosedLoopRunner:
    ...
    def run_dataset(
        ...
        *,
        ...
        if version_upper.startswith("V9") and not self.config.no_viability_kernel and not self.config.v2_reference_runtime and not self.config.v5_reference_runtime and not self.config.v6_reference_runtime and not self.config.v7_reference_runtime and not self.config.v8_reference_runtime:
        ...
        eval_semantics = {
            ...
            "capability_viability_kernel_enabled": bool(any(str(self.config.algorithm_version).upper().startswith(v) for v in ("V5","V6","V7","V8","V9","V10")) and not self.config.no_viability_kernel and not self.config.v2_reference_runtime),
            "typed_viability_pruning_enabled": bool(any(str(self.config.algorithm_version).upper().startswith(v) for v in ("V5","V6","V7","V8","V9","V10")) and not self.config.no_viability_kernel and not self.config.no_typed_viability and not self.config.v2_reference_runtime),
            "precondition_antichain_enabled": bool(any(str(self.config.algorithm_version).upper().startswith(v) for v in ("V6","V7","V8","V9","V10")) and not self.config.no_viability_kernel and not self.config.no_precondition_antichain and not self.config.v5_reference_runtime and not self.config.v2_reference_runtime),
            "viability_proof_envelope_enabled": bool((str(self.config.algorithm_version).upper().startswith("V6") or str(self.config.algorithm_version).upper().startswith("V7") or ((str(self.config.algorithm_version).upper().startswith("V8") or str(self.config.algorithm_version).upper().startswith("V9") or str(self.config.algorithm_version).upper().startswith("V10")) and self.config.v7_reference_runtime)) and not self.config.no_viability_kernel and not self.config.no_viability_proof_envelope and not self.config.v5_reference_runtime and not self.config.v2_reference_runtime),
            "direct_precondition_compilation_enabled": bool(any(str(self.config.algorithm_version).upper().startswith(v) for v in ("V7","V8","V9","V10")) and not self.config.no_viability_kernel and not self.config.v6_reference_runtime and not self.config.v5_reference_runtime and not self.config.v2_reference_runtime),
            ...
            "capability_projection_enabled": bool((str(self.config.algorithm_version).upper().startswith("V9") or str(self.config.algorithm_version).upper().startswith("V10")) and not self.config.v8_reference_runtime and not self.config.no_capability_projection and not self.config.no_viability_kernel and not self.config.v2_reference_runtime and not self.config.v5_reference_runtime and not self.config.v6_reference_runtime and not self.config.v7_reference_runtime),
            "frontier_signature_index_enabled": bool((str(self.config.algorithm_version).upper().startswith("V9") or str(self.config.algorithm_version).upper().startswith("V10")) and not self.config.v8_reference_runtime and not self.config.no_frontier_signature_index and not self.config.no_viability_kernel and not self.config.v2_reference_runtime and not self.config.v5_reference_runtime and not self.config.v6_reference_runtime and not self.config.v7_reference_runtime),
            "semnaive_projected_kernel_enabled": bool(str(self.config.algorithm_version).upper().startswith("V10") and not self.config.v9_reference_runtime and not self.config.no_viability_kernel and not self.config.v2_reference_runtime and not self.config.v5_reference_runtime and not self.config.v6_reference_runtime and not self.config.v7_reference_runtime and not self.config.v8_reference_runtime),
            "semnaive_delta_propagation_enabled": bool(str(self.config.algorithm_version).upper().startswith("V10") and not self.config.v9_reference_runtime and not self.config.no_semnaive_delta_propagation),
            "packed_frontier_dominance_enabled": bool(str(self.config.algorithm_version).upper().startswith("V10") and not self.config.v9_reference_runtime and not self.config.no_packed_frontier_dominance),
            ...
            "proof_carrying_viability_certificates": bool(
                (
                    ...
                    or ((str(self.config.algorithm_version).upper().startswith("V8") or str(self.config.algorithm_version).upper().startswith("V9") or str(self.config.algorithm_version).upper().startswith("V10")) and not self.config.no_lazy_diagnostic_replay)
                )
                and not self.config.no_viability_kernel
