...
def result_to_episode_metrics(
    ...
    return {
        ...
        "precondition_rejection_antichain_size": int(result.diagnostics.get("precondition_rejection_antichain_size", 0) or 0),
        ...
        "projected_resource_count": int(result.diagnostics.get("projected_resource_count", 0) or 0),
        "projected_evidence_dropped": int(result.diagnostics.get("projected_evidence_dropped", 0) or 0),
        "frontier_signature_hits": int(result.diagnostics.get("frontier_signature_hits", 0) or 0),
        "frontier_dominance_checks": int(result.diagnostics.get("frontier_dominance_checks", 0) or 0),
        "frontier_peak_size": int(result.diagnostics.get("frontier_peak_size", 0) or 0),
        "delta_propagations": int(result.diagnostics.get("delta_propagations", 0) or 0),
        "delta_admissions": int(result.diagnostics.get("delta_admissions", 0) or 0),
        "delta_stale_skips": int(result.diagnostics.get("delta_stale_skips", 0) or 0),
        "frontier_mask_rejects": int(result.diagnostics.get("frontier_mask_rejects", 0) or 0),
        "frontier_packed_fastpath": int(result.diagnostics.get("frontier_packed_fastpath", 0) or 0),
        "frontier_packed_fallbacks": int(result.diagnostics.get("frontier_packed_fallbacks", 0) or 0),
        "native_projected_compositions": int(result.diagnostics.get("native_projected_compositions", 0) or 0),
        "native_projected_materializations": int(result.diagnostics.get("native_projected_materializations", 0) or 0),
        "native_projected_fallbacks": int(result.diagnostics.get("native_projected_fallbacks", 0) or 0),
        "fused_frontier_passes": int(result.diagnostics.get("fused_frontier_passes", 0) or 0),
        "precondition_build_ms": float(result.diagnostics.get("precondition_build_ms", 0.0) or 0.0),
        "parametric_kernel_cache_hit": int(result.diagnostics.get("parametric_kernel_cache_hit", 0) or 0),
        "parametric_kernel_cache_miss": int(result.diagnostics.get("parametric_kernel_cache_miss", 0) or 0),
        "parametric_kernel_cache_entries": int(result.diagnostics.get("parametric_kernel_cache_entries", 0) or 0),
        "parametric_kernel_thresholds_erased": int(result.diagnostics.get("parametric_kernel_thresholds_erased", 0) or 0),
        "parametric_kernel_lookup_ms": float(result.diagnostics.get("parametric_kernel_lookup_ms", 0.0) or 0.0),
        "parametric_kernel_source_build_ms": float(result.diagnostics.get("parametric_kernel_source_build_ms", 0.0) or 0.0),
        "parametric_kernel_shape_id": str(result.diagnostics.get("parametric_kernel_shape_id", "") or ""),
        ...
        "diagnostic_semantic_cache_enabled": bool(result.diagnostics.get("diagnostic_semantic_cache_enabled", False)),
        "diagnostic_semantic_cache_hits": int(result.diagnostics.get("diagnostic_semantic_cache_hits", 0) or 0),
        "diagnostic_semantic_cache_misses": int(result.diagnostics.get("diagnostic_semantic_cache_misses", 0) or 0),
        "diagnostic_semantic_cache_entries": int(result.diagnostics.get("diagnostic_semantic_cache_entries", 0) or 0),
        "diagnostic_semantic_cache_primary_stores": int(result.diagnostics.get("diagnostic_semantic_cache_primary_stores", 0) or 0),
        "diagnostic_compiled_program_enabled": bool(result.diagnostics.get("diagnostic_compiled_program_enabled", False)),
        "diagnostic_compiled_program_entries": int(result.diagnostics.get("diagnostic_compiled_program_entries", 0) or 0),
        "diagnostic_compiled_program_hits": int(result.diagnostics.get("diagnostic_compiled_program_hits", 0) or 0),
        "diagnostic_compiled_program_misses": int(result.diagnostics.get("diagnostic_compiled_program_misses", 0) or 0),
        "diagnostic_compiled_program_compiles": int(result.diagnostics.get("diagnostic_compiled_program_compiles", 0) or 0),
...
class ClosedLoopRunner:
    ...
    def run_dataset(
        ...
        *,
        ...
        if version_upper.startswith("V16") and not self.config.no_viability_kernel and not self.config.v2_reference_runtime:
            frontier_guidance_policy = "parametric_capability_kernel_v16" if self.config.use_parametric_capability_kernel else "exact_no_learning_sn_cpk"
        elif version_upper.startswith("V15") and not self.config.no_viability_kernel and not self.config.v2_reference_runtime:
            if self.config.v15_legacy_static_guidance:
                frontier_guidance_policy = "historical_edge_plus_static_learned_ordering_v15_control"
            elif self.config.no_exact_robustness_ordering:
                frontier_guidance_policy = "exact_no_ordering_semnaive_projected_acceptance_v15"
            elif self.config.exact_robustness_count_only:
                frontier_guidance_policy = "exact_summary_count_ordering_semnaive_projected_acceptance_v15_control"
            else:
                frontier_guidance_policy = "exact_capability_robustness_ordering_semnaive_projected_acceptance_v15"
        elif version_upper.startswith("V14") and not self.config.no_viability_kernel and not self.config.v2_reference_runtime:
        ...
        elif version_upper.startswith("V10") and not self.config.no_viability_kernel and not self.config.v2_reference_runtime and not self.config.v5_reference_runtime and not self.config.v6_reference_runtime and not self.config.v7_reference_runtime and not self.config.v8_reference_runtime and not self.config.v9_reference_runtime:
            frontier_guidance_policy = "semnaive_capability_projected_acceptance_lazy_exact_proof_v10"
        elif version_upper.startswith("V9") and not self.config.no_viability_kernel and not self.config.v2_reference_runtime and not self.config.v5_reference_runtime and not self.config.v6_reference_runtime and not self.config.v7_reference_runtime and not self.config.v8_reference_runtime:
            frontier_guidance_policy = "capability_projected_incremental_acceptance_lazy_exact_proof_v9"
        elif (version_upper.startswith("V8") or (version_upper.startswith("V9") and self.config.v8_reference_runtime)) and not self.config.no_viability_kernel and not self.config.v2_reference_runtime and not self.config.v5_reference_runtime and not self.config.v6_reference_runtime and not self.config.v7_reference_runtime:
            frontier_guidance_policy = "incremental_acceptance_lazy_exact_proof_v8"
        elif (version_upper.startswith("V7") or ((version_upper.startswith("V8") or version_upper.startswith("V9")) and self.config.v7_reference_runtime)) and not self.config.no_viability_kernel and not self.config.v2_reference_runtime and not self.config.v5_reference_runtime and not self.config.v6_reference_runtime:
            frontier_guidance_policy = "direct_asymmetric_dual_precondition_kernel_v7"
        elif (version_upper.startswith("V6") or ((version_upper.startswith("V7") or version_upper.startswith("V8") or version_upper.startswith("V9")) and self.config.v6_reference_runtime)) and not self.config.no_viability_kernel and not self.config.v2_reference_runtime and not self.config.v5_reference_runtime:
            frontier_guidance_policy = "proof_carrying_weakest_precondition_antichain_v6"
        elif (version_upper.startswith("V5") or ((version_upper.startswith("V6") or version_upper.startswith("V7") or version_upper.startswith("V8") or version_upper.startswith("V9")) and self.config.v5_reference_runtime)) and not self.config.no_viability_kernel and not self.config.v2_reference_runtime:
        ...
        elif version_upper.startswith("V3") and self.config.frontier_ranker_checkpoint and not self.config.no_frontier_ranker and not self.config.v2_reference_runtime:
            frontier_guidance_policy = "executable_capability_frontier_v3"
        else:
            frontier_guidance_policy = "v2_static_typed_feasibility" if self.config.v2_reference_runtime else "none_or_legacy"
        eval_semantics = {
            ...
            "frontier_guidance_policy": frontier_guidance_policy,
            ...
            "capability_viability_kernel_enabled": bool(any(str(self.config.algorithm_version).upper().startswith(v) for v in ("V5","V6","V7","V8","V9","V10","V11","V12","V13","V14","V15","V16")) and not self.config.no_viability_kernel and not self.config.v2_reference_runtime),
            "typed_viability_pruning_enabled": bool(any(str(self.config.algorithm_version).upper().startswith(v) for v in ("V5","V6","V7","V8","V9","V10","V11","V12","V13","V14","V15","V16")) and not self.config.no_viability_kernel and not self.config.no_typed_viability and not self.config.v2_reference_runtime),
            "precondition_antichain_enabled": bool(any(str(self.config.algorithm_version).upper().startswith(v) for v in ("V6","V7","V8","V9","V10","V11","V12","V13","V14","V15","V16")) and not self.config.no_viability_kernel and not self.config.no_precondition_antichain and not self.config.v5_reference_runtime and not self.config.v2_reference_runtime),
            "viability_proof_envelope_enabled": bool((str(self.config.algorithm_version).upper().startswith("V6") or str(self.config.algorithm_version).upper().startswith("V7") or ((str(self.config.algorithm_version).upper().startswith("V8") or str(self.config.algorithm_version).upper().startswith("V9") or str(self.config.algorithm_version).upper().startswith("V10") or str(self.config.algorithm_version).upper().startswith("V11") or str(self.config.algorithm_version).upper().startswith("V12") or str(self.config.algorithm_version).upper().startswith("V13") or str(self.config.algorithm_version).upper().startswith("V14") or str(self.config.algorithm_version).upper().startswith(("V15","V16"))) and self.config.v7_reference_runtime)) and not self.config.no_viability_kernel and not self.config.no_viability_proof_envelope and not self.config.v5_reference_runtime and not self.config.v2_reference_runtime),
            ...
            "legacy_static_learned_guidance_enabled": bool((not self.config.no_learned_feasibility_guidance) and (
                (not str(self.config.algorithm_version).upper().startswith(("V11", "V12", "V13", "V14", "V15","V16")))
            ...
            "shared_exact_diagnostic_semantic_cache_enabled": bool(
                str(self.config.algorithm_version).upper().startswith("V12")
                and not self.config.no_shared_diagnostic_semantic_cache
                and not self.config.v11_reference_runtime and not self.config.v10_reference_runtime
                and not self.config.v9_reference_runtime and not self.config.v8_reference_runtime
                and not self.config.v7_reference_runtime and not self.config.v6_reference_runtime
            ...
            "v16_parametric_capability_kernel_enabled": bool(version_upper.startswith("V16") and self.config.use_parametric_capability_kernel),
            "v16_parametric_kernel_full_contract_key": bool(version_upper.startswith("V16") and self.config.parametric_kernel_full_contract_key),
            "v16_contract_shape_semantics": ("erase_only_cumulative_upper_lower_numeric_thresholds_v1" if version_upper.startswith("V16") else None),
