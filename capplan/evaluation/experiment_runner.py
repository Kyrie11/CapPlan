...
def run_main(dataset_dir: str | Path, output_dir: str | Path) -> Dict[str, float]:
    res = ClosedLoopRunner().run_dataset(dataset_dir, Path(output_dir) / "main")
    write_csv(Path(output_dir) / "main_results.csv", {"CapPlan": res["metrics"]})
    return res["metrics"]


def run_ablation_table(dataset_dir: str | Path, output_dir: str | Path) -> Dict[str, Dict[str, float]]:
    rows = run_ablations(dataset_dir, Path(output_dir) / "ablations")
