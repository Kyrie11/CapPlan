    ...
    def __init__(self, dataset_dir: str | Path, split: str = "train", vocab: FeatureVocab | None = None) -> None:
        ...
        for d in read_jsonl(self.dataset_dir / "candidate_transitions.jsonl"):
            ...
            if not contracts:
                ...
                self.samples.append(CASASample(t.transition_id, t.episode_id, "__transition_only__", encode_transition_with_capability(t, [], self.vocab), y_edge, y_value, y_phase))
                continue
            for contract in contracts:
                ...
                if in_skeleton:
                    ...
                    y_value = 0.0
                y_phase = self.vocab.phases.index(t.to_phase) if t.to_phase in self.vocab.phases else 0
                self.samples.append(CASASample(t.transition_id, t.episode_id, contract.passenger_id, encode_transition_with_capability(t, compiled.tokens, self.vocab), y_edge, y_value, y_phase))
