...
def _require_artifact(path: Path, label: str, dry_run: bool) -> None:
    """Fail fast on missing *internal pipeline artifacts* without re-parsing them.

    External GIS/DEM/source semantics are validated by the explicit preflight
    path above.  This helper is used only for artifacts generated by CapPlan
    itself (scene JSONL, graph directories, PUDO/service JSONL, allowlists and
    dataset manifests).  Calling :func:`inspect_source` here was both incorrect
    for plain-text allowlists and catastrophically expensive for graph
    directories because it recursively parsed every JSONL record.
    """
    if dry_run:
        return
    p = Path(path)
    if not p.exists():
        raise RuntimeError(f"{label} is missing: {p}")
    if p.is_dir():
        # Accessibility-graph directories contain many very large JSONL files.
        # A generated-artifact gate only needs a non-empty witness; downstream
        # loaders/strict validators perform schema and semantic checks.  Keep
        # this lazy so the check is O(first file), not O(all graph records).
        witness = None
    ...
    if not p.is_file() or p.stat().st_size <= 0:
        raise RuntimeError(f"{label} is empty or not a regular file: {p}")
    if p.suffix.lower() == ".txt":
        # Episode allowlists are intentionally plain text.  Require at least one
        # real ID (comments/blank lines do not count), but do not send them
        # through the external-source suffix validator.
        with p.open("r", encoding="utf-8") as f:
            if not any(line.strip() and not line.lstrip().startswith("#") for line in f):
                raise RuntimeError(f"{label} contains no usable entries: {p}")
