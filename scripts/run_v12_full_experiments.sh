...
V12_ROOT="${V12_ROOT:-$CAP_HOME/outputs/eval/v12_full_seed${SEED}}"
...
CUDA_VISIBLE_DEVICES="$GPU0" python scripts/run_ablations.py \
  ...
  --progress > >(tee "$V12_ROOT/logs/v12_full.log") 2>&1
...
python scripts/summarize_v12_results.py \
  ...
  --output "$V12_ROOT/v12_full_summary.csv" | tee "$V12_ROOT/logs/summary.log"
...
python scripts/assess_v12_fast.py \
  ...
  --output "$V12_ROOT/v12_full_gate.json" | tee "$V12_ROOT/logs/gate.log"

echo "V12_FULL_ROOT=$V12_ROOT"
echo "V12 full is the exact-backbone freeze gate. Only status == GO permits CQ-HPT to become the next main algorithm version."
