...
V13_ROOT="${V13_ROOT:-$CAP_HOME/outputs/eval/v13_full_seed${SEED}}"
...
CUDA_VISIBLE_DEVICES="$GPU0" python scripts/run_ablations.py \
  ...
  --progress > >(tee "$V13_ROOT/logs/v13_full.log") 2>&1
...
python scripts/summarize_v13_results.py \
  ...
  --output "$V13_ROOT/v13_full_summary.csv" | tee "$V13_ROOT/logs/summary.log"
...
python scripts/assess_v13_fast.py \
  ...
  --output "$V13_ROOT/v13_full_gate.json" | tee "$V13_ROOT/logs/gate.log"

echo "V13_FULL_ROOT=$V13_ROOT"
echo "V13 full is the symbolic-backbone closure gate. Only full GO freezes execution/diagnosis representation before CQ-HPT training."
