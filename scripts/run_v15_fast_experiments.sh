...
CASA_CHECKPOINT="${CASA_CHECKPOINT:-$CAP_HOME/outputs/models/casa_relation_mlp_seed13/checkpoint.pt}"; V15_ROOT="${V15_ROOT:-$CAP_HOME/outputs/eval/v15_fast_seed${SEED}}"
...
python scripts/assess_v15_fast.py --full "$V15_ROOT/ecrp_full" --exact "$V15_ROOT/exact_no_ordering" --legacy "$V15_ROOT/legacy_static" --count-only "$V15_ROOT/summary_count_only" \
  --full-exact "$V15_ROOT/full_vs_exact.json" --full-legacy "$V15_ROOT/full_vs_legacy.json" --full-count "$V15_ROOT/full_vs_count.json" --exactness "$V15_ROOT/full_vs_exact_diagnostic.json" --repeat-primary "$V15_ROOT/repeated_primary_exact_vs_ecrp.json" --output "$V15_ROOT/v15_fast_gate.json"
cat "$V15_ROOT/v15_fast_gate.json"
