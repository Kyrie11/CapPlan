...
DATASET_DIR="${DATASET_DIR:-$DATA_ROOT/outputs/datasets/abilitybench_av_hybrid_test}"; CASA_CHECKPOINT="${CASA_CHECKPOINT:-$CAP_HOME/outputs/models/casa_relation_mlp_seed13/checkpoint.pt}"; V16_ROOT="${V16_ROOT:-$CAP_HOME/outputs/eval/v16_fast_seed${SEED}}"
EPISODE_LIMIT="${EPISODE_LIMIT:-256}"; EPISODE_SEED="${EPISODE_SEED:-13}"
mkdir -p "$V16_ROOT/logs"; cd "$CAP_HOME"
runv(){ local name="$1" mode="$2"; CUDA_VISIBLE_DEVICES="$GPU0" python scripts/run_v16_eval_variant.py --dataset_dir "$DATASET_DIR" --output_dir "$V16_ROOT/$name" --casa_checkpoint "$CASA_CHECKPOINT" --mode "$mode" --episode_limit "$EPISODE_LIMIT" --episode_seed "$EPISODE_SEED" --show_progress | tee "$V16_ROOT/logs/${name}.log"; }
...
# Four counterbalanced serial repeats. The only changing mechanism is exact PCK reuse.
for r in 0 1 2 3; do
